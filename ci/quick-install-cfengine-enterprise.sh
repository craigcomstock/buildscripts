...
case $DISTRO in
    Ubuntu)
        ...
        if [[ "$uname_arch" == "aarch64" ]] && [[ "$MAJOR_RELEASE" -ge "22" ]]; then
            folder_arch_suffix="arm_64"
            folder="ubuntu_${MAJOR_RELEASE}_${folder_arch_suffix}"
            comp_tag="ubuntu${MAJOR_RELEASE}"
        elif [ "$MAJOR_RELEASE" -eq 24 ]; then
            # Ubuntu 24
            folder="ubuntu_24_x86_64"
            comp_tag="ubuntu24"
        elif [ "$MAJOR_RELEASE" -eq 22 ]; then
            # Ubuntu 22
    ...
    Debian)
        ...
        if [ "$MAJOR_RELEASE" -lt 7 ]; then
            ...
            folder_arch_suffix="arm_64"
            folder="debian_${MAJOR_RELEASE}_${folder_arch_suffix}"
            ...
            comp_tag="debian10"
        elif [ "$MAJOR_RELEASE" -eq 11 ]; then
            folder="debian_11_x86_64"
            comp_tag="debian11"
        elif [ "$MAJOR_RELEASE" -eq 12 ]; then
            folder="debian_12_x86_64"
            comp_tag="debian12"
        elif [ "$MAJOR_RELEASE" -gt 12 ]; then
            folder="debian_12_x86_64"
            comp_tag="debian12"
            echo "Sorry, we do not officially support hub functionality on $DISTRO $MAJOR_RELEASE."
            echo "Installing the package that is most likely to work. We recommend using cf-remote https://github.com/cfengine/cf-remote."
...
case $DISTRO in
    Ubuntu)
        ...
        if [[ "$uname_arch" != "x86_64" ]] && [[ "$uname_arch" != "aarch64" ]]; then
            ...
            comp_tag="ubuntu${MAJOR_RELEASE}"
        elif [ "$MAJOR_RELEASE" -ge "24" ]; then
            # Ubuntu 24+
            folder="agent_ubuntu24_x86_64"
            comp_tag="ubuntu24"
    ...
    Debian)
        ...
        if [[ "$uname_arch" != "x86_64" ]] && [[ "$uname_arch" != "aarch64" ]]; then
            ...
            # Debian 11
            folder="agent_debian11_x86_64"
            comp_tag="debian11"
        elif [ "$MAJOR_RELEASE" -eq 12 ]; then
            # Debian 12
            folder="agent_debian12_x86_64"
            comp_tag="debian12"
        elif [ "$MAJOR_RELEASE" -gt 12 ]; then
            # Debian 12+
            folder="agent_debian12_x86_64"
    ...
    RedHatEnterprise*|SUSE|CentOS|AmazonLinux|OracleLinux)
	    ...
	    if [ "$MAJOR_RELEASE" -ge 15 ]; then
		folder="agent_suse15_x86_64"
		comp_tag="suse15"
	    else
		folder="agent_suse12_x86_64"
		comp_tag="suse12"
	    fi
        else
