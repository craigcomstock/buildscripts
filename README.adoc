...
A pre-requisite is to have necessary repositories checked out along-side each other.

A minimal example would be to build packages for cfengine community agent.

```
include::checkout-community-agent.sh[]
```
...
And execute the build steps:

```
include::build-community-agent.sh[]
```
